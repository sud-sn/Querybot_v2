...
# build_duckdb_system_prompt lived here: 112 lines that composed the SQL
# generation prompt for in-memory DuckDB queries, injecting the cached result's
# statistical summary -- min, max, average, categorical values -- into an LLM
# call.
#
# A governance change removed it from the cached-result path, and nothing has
# called it since. It stayed because deleting code feels riskier than keeping
# it; the opposite is true here. The whole point of that change was that the
# result-cache planner forwards NO cached values to a model, and this function
# is a ready-made way to undo it: importable, documented, and looking
# maintained. tests/test_regulated_block_audit_trail.py asserts the planner
# does not call it, which is why its absence is now asserted by name below
# rather than by a substring that a deleted symbol makes vacuously true.
