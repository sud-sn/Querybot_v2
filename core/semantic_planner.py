...
def _find_candidates(
    ...
    return _scope_to_explicitly_named_sources(candidates, qn, table_columns)


# Name fragments that identify a table's layer, not the data inside it.
_TABLE_LAYER_TOKENS = frozenset({
    "D", "F", "B", "BR", "DIM", "FACT", "FCT", "DMS", "TBL", "TMP", "STG", "VW",
})


def _source_signal_tokens(table_columns: dict[str, dict[str, str]]) -> dict[str, set[str]]:
    """Tokens that name a *source*, never a business concept.

    A token qualifies only if it appears in a table's name and in no column
    anywhere in the schema. That distinction is the whole point: WAREHOUSE
    names D_WAREHOUSE but is also WAREHOUSE_SK / WAREHOUSE_NAME, so scoping on
    it would drop the measure from "inventory value by warehouse". MITBAL and
    M3 appear only in M3_MITBAL, so a question containing them is naming a
    system, not describing a quantity.
    """
    column_tokens: set[str] = set()
    for cols in (table_columns or {}).values():
        for col in (cols or {}):
            column_tokens.update(t for t in str(col).upper().split("_") if t)

    signals: dict[str, set[str]] = {}
    for table in (table_columns or {}):
        bare = str(table).upper().split(".")[-1]
        tokens = {
            token for token in bare.split("_")
            if len(token) >= 2
            and token not in _TABLE_LAYER_TOKENS
            and token not in column_tokens
        }
        if tokens:
            signals[str(table).upper()] = tokens
    return signals


def _scope_to_explicitly_named_sources(
    candidates: list[dict],
    normalized_question: str,
    table_columns: dict[str, dict[str, str]],
) -> list[dict]:
    """Honour a source the user named outright.

    Live case 16 asked "using the M3 balance data ..." and was resolved against
    the daily snapshot fact instead; case 4 ignored the client's own KB rule
    about which table anchors a relative period. Candidate scoring compares a
    term against column names only, so naming the table changed nothing.

    When the question carries a source signal, keep only candidates from the
    tables it names. Silent no-op otherwise -- no signal, or a signal that
    would empty the candidate list, leaves scoring exactly as it was.
    """
    signals = _source_signal_tokens(table_columns)
    if not signals:
        return candidates
    # _norm() lowercases; signal tokens come from table names in upper case.
    question_tokens = {t.upper() for t in normalized_question.split()}
    named = {
        table for table, tokens in signals.items()
        if tokens & question_tokens
    }
    if not named:
        return candidates
    scoped = [c for c in candidates if str(c.get("table") or "").upper() in named]
    if not scoped:
        # The named source carries none of the requested terms. Refusing to
        # scope beats returning nothing: the caller still needs candidates to
        # score, and a wrong hard scope would reject a correct answer.
        log.info(
            "Source scoping skipped: question names %s but no candidate term "
            "resolves there", sorted(named),
        )
        return candidates
    if len(scoped) != len(candidates):
        log.info(
            "Scoped candidates to source(s) named in the question: %s "
            "(%d of %d candidates kept)",
            sorted(named), len(scoped), len(candidates),
        )
    return scoped
