...
Both of the above are wired: core/insight.py:145 detects the intent and
core/query_pipeline.py:4728 injects the hint, so the warehouse computes the
window function and the result is governed like any other query.

NOT entry points, and this list used to say they were
─────────────────────────────────────────────────────
  compute_rolling_average, compute_running_total, compute_row_delta,
  compute_rank_in_group

They are a complete in-Python alternative to the SQL path, and nothing outside
tests/test_analytics_smoke.py calls any of them. That is a deliberate design
choice — the module's own philosophy note above says the SQL is generated by
the LLM — but the docstring advertised them as this module's post-execution
API, and four passing tests made them look live. A reader had every reason to
believe rolling averages were computed here.

They are kept rather than deleted: they are correct, tested, and the obvious
implementation if a dialect ever cannot express a window the plan needs. What
is not kept is the claim that anything calls them.
"""
