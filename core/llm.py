...
def build_sql_system_prompt(
    ...
    base = (
        ...
        "- NULL-AWARE FILTERED AGGREGATE RULE: When the user asks for a single metric filtered "
        "to a specific KEY, ENTITY, or CATEGORY that might not exist or might match no rows "
        "(for example revenue for customer 123, cost for item X, quantity for warehouse Y, "
        "revenue for status = 'Cancelled'), include diagnostics so the answer can distinguish "
        "true zero from missing data: COUNT_BIG(*) AS [MatchedRows] on Azure SQL "
        "(COUNT(*) AS MatchedRows on other DBs), COUNT(metric_col) AS [NonNullMetricRows], "
        "and COALESCE(SUM(metric_col), 0) AS [MetricName]. "
        "Do NOT apply this to a plain time-bounded aggregate ('sales for the last 7 days', "
        "'revenue this month', 'total orders this year') — a date-range WHERE clause has no "
        "'does this entity exist' ambiguity, so a bare SUM(metric_col) is correct there; adding "
        "these diagnostic columns to a simple time-filtered total is wrong and breaks the "
        "single-value answer.\n\n"
