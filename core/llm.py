...
def build_sql_system_prompt(
    ...
    if db_type == "azure_sql":
        ...
        _mom_pattern = (
            ...
            "  SELECT <native_or_governed_period_expression> AS PERIOD, <approved_formula> AS METRIC\n"
            "  FROM [schema].[fact] f\n"
            "  <exact governed date-role JOIN when required>\n"
            "  WHERE <requested date range and approved filters only>\n"
            "  GROUP BY <native_or_governed_period_expression>\n"
            "), period_comparison AS (\n"
            "  SELECT PERIOD, METRIC, LAG(METRIC) OVER (ORDER BY PERIOD) AS PREV_METRIC\n"
            "  FROM period_totals\n"
            ")\n"
            ...
            "FROM period_comparison ORDER BY PERIOD"
        )
        ...
        _mom_pattern = (
            ...
            "SELECT PERIOD, METRIC, PREV_METRIC,\n"
            "       METRIC - PREV_METRIC AS DIFF,\n"
            "       ROUND((METRIC - PREV_METRIC) * 100 / NULLIF(PREV_METRIC, 0), 2) AS PCT_CHANGE\n"
            "FROM period_comparison ORDER BY PERIOD"
        )
        _top_n_per_group_pattern = (
            "WITH ranked AS (\n"
    ...
    base = (
        ...
        f"{_xjoin_pattern}\n"
        "- SURROGATE KEY DISPLAY RULE: A surrogate or foreign key (a column "
        "ending _SK, _KEY, _ID, _FK or _DMS_KEY) is a join key, never an "
        "answer. Grouping by one on the fact table runs without error but "
        "reports meaningless integers to the user. When you GROUP BY such a "
        "column and the schema above has a dimension holding that same key "
        "plus a name or description column (ending _NAME, _DESC, _DSC, "
        "_DESCRIPTION or _NM), you MUST join that dimension and both SELECT "
        "and GROUP BY the name column instead of the key. Only return the raw "
        "key when the user explicitly asked for the key, id or code, or when "
        "the schema above has no dimension carrying that key with a name "
        "column — never invent a table or column that is not listed.\n"
        ...
        + (
            ...
            "calendar column directly on a fact table is invalid SQL — the column does not "
            "exist there. To filter or group a fact table by year/month/quarter, either:\n"
        ...
        + (
            ...
            "DMS-key rule above — applies to columns it does NOT cover, and its "
            "TRY_CONVERT technique is WRONG here — do not blend the two): a fact-table "
            ...
            "it has NO inherent calendar meaning and is NOT YYYYMMDD-encoded. Its "
            "integer value (e.g. 116806) is NOT an 8-digit date string, so every "
            "technique below is a real bug that returns wrong or zero rows with no "
            "error, not just a style issue:\n"
            "    WRONG: WHERE fact.SOME_DATE_ID BETWEEN 20260101 AND 20261231 "
            "(comparing to a literal)\n"
            "    WRONG: DATEADD(...), MONTH(fact.SOME_DATE_ID) (arithmetic directly "
            "on the key)\n"
            "    WRONG: YEAR(TRY_CONVERT(date, CONVERT(varchar(8), "
            "fact.SOME_DATE_ID), 112)) — this is the DMS-key rule's technique; it "
            "is ONLY valid for a genuinely YYYYMMDD-encoded integer. Applying it "
            "here silently produces NULL for every row (CONVERT(varchar(8), "
            "116806) is a 6-character string, not 8 digits, so TRY_CONVERT fails "
            "for every row) — the query runs, returns zero rows, and looks like a "
            "data problem instead of the real bug.\n"
            "    WRONG: inventing a column name with a _DMS_KEY / _KEY suffix that "
            "isn't in the schema context (e.g. renaming SOME_DATE_ID to "
            "SOME_DATE_DMS_KEY) to make the technique above 'fit' — the column "
            "name in the schema context is the only name that exists; never alter "
            "its suffix to match a different rule's naming convention.\n"
            "  CORRECT: you MUST JOIN to the date dimension (use the exact FK shown "
            "in the entity-graph JOIN skeleton when one is provided) and filter/group "
            "using the DIMENSION table's own real calendar columns — look up "
            "their actual names in the schema context for that table (a year "
            "column, month column, quarter column, or native date column). The "
            "fact-side surrogate key is only ever used in the JOIN ON clause, "
            "never in a WHERE/GROUP BY expression by itself.\n"
        ...
        "    )\n"
        "  Azure SQL date-key pattern:\n"
        "    WITH dated AS (\n"
        "      SELECT CAST(YEAR(TRY_CONVERT(date, CONVERT(varchar(8), alias.DATE_KEY_COL), 112)) AS INT) AS YR,\n"
        "             metric_col\n"
        "      FROM [schema].[table] alias\n"
        "      WHERE alias.DATE_KEY_COL > 0\n"
        "    ), base AS (\n"
        "      SELECT YR, SUM(metric_col) AS METRIC\n"
        "      FROM dated\n"
        "      GROUP BY YR\n"
        "    )\n"
        "    Never write CAST(YEAR(TRY_CONVERT(...))) AS INT); the AS INT belongs inside CAST(... AS INT).\n"
        ...
        "SELECT. Do not CROSS JOIN physical tables and do not use a comma join. A CROSS JOIN to "
        "a separate benchmark relation is acceptable only when that relation is guaranteed to "
        ...
        "LAG(METRIC) to that stable alias in a comparison CTE, then calculate DIFF/PCT_CHANGE "
        "in the final SELECT. Never write LAG(SUM(...)) or repeat the aggregate inside LAG.\n"
        "  DATE TYPE RULE: If the semantic/date-role plan provides a native DATE/DATETIME value "
        "such as date_dimension.CALENDAR_DATE, bucket that column directly. For Azure SQL month "
        "bucketing use DATEFROMPARTS(YEAR(date_col), MONTH(date_col), 1). Only use "
        "TRY_CONVERT(date, CONVERT(varchar(8), key_col), 112) when schema metadata proves the "
        "selected value itself is a numeric YYYYMMDD key. Never convert an already-native date "
        "through varchar(8).\n"
        "  DATE ROLE IS NOT STATUS: wording such as 'booked month', 'paid month', or 'dispensed "
        "month' selects the governed role-playing date JOIN. Do not add STATUS = 'BOOKED', "
        "STATUS = 'PAID', or a similar status predicate unless the user explicitly requests that "
        "status or an approved metric/entity filter requires it.\n"
        "  Use this dialect-specific structure:\n"
        f"{_mom_pattern}\n"
        ...
        "- NULL-AWARE FILTERED AGGREGATE RULE — run this exact decision test before writing "
        "a single-metric aggregate query. Do not skip step 1.\n"
        "  STEP 1 — is the WHERE clause a TIME-ONLY filter (a date column compared with a "
        "range, BETWEEN, or 'last N days' / 'this month' / 'this year' style window, with "
        "nothing else filtered)? If yes: write a PLAIN aggregate and STOP — "
        "SELECT SUM(metric_col) AS MetricName FROM ... WHERE date_col >= X. "
        "Do NOT add COUNT_BIG(*), COUNT(*), [MatchedRows], [NonNullMetricRows], or a "
        "COALESCE/ISNULL wrapper around the SUM — none of that belongs on a time-only filter. "
        "Worked example — question: 'what is my sales for the last 7 days' → "
        "SELECT SUM(SalesAmt) AS TotalSales FROM Orders "
        "WHERE OrderDate >= DATEADD(day, -7, ...) — three columns (MatchedRows/"
        "NonNullMetricRows/TotalSales) would be WRONG here.\n"
        "  STEP 2 — only reached if step 1 was no: does the WHERE clause filter to a "
        "SPECIFIC KEY, ENTITY, or CATEGORY that might not exist or might match zero rows "
        "(a customer id, item code, warehouse name, or a status/category value — e.g. "
        "revenue for customer 123, cost for item X, revenue for status = 'Cancelled')? If "
        "yes: THIS is the case that needs diagnostics, because the answer must be able to "
        "tell 'no such record' apart from 'the value is genuinely zero'. Add "
        "COUNT_BIG(*) AS [MatchedRows] on Azure SQL (COUNT(*) AS MatchedRows on other DBs), "
        "COUNT(metric_col) AS [NonNullMetricRows], and COALESCE(SUM(metric_col), 0) AS "
        "[MetricName].\n"
        "  A question can combine both ('revenue for customer 123 in the last 7 days') — "
        "that still needs step 2's diagnostics, because the entity filter is present "
        "alongside the date filter. But a time-only filter by itself never gets the "
        "diagnostic columns, regardless of how the question is phrased ('filtered', 'for', "
        "'with') — wording alone does not trigger step 2, only an actual entity/key/"
        "category condition in the WHERE clause does.\n\n"
...
def build_kb_system_prompt(erp_hints: str = "", naming_hints: str = "") -> str:
    ...
    if naming_hints:
        naming_block = (
            ...
            "  - ## Always Exclude: list AZ_/ETL_/DW_ audit/infrastructure columns as 'never use in business queries'. Do NOT add soft-delete indicator columns here — they are optional user-driven filters.\n"
    ...
    return (
        ...
        "you MUST list those exact values and use them in examples, and never "
        "invent one. Describe them as the values OBSERVED when the schema was "
        "read — the document states that date — never as the complete or only "
        "possible set. They are a snapshot: the table may hold values added "
        "since, and a question about one of those is still a valid question.\n"
        ...
        "## Always Exclude\n"
        "Columns that must NEVER appear in business query output and have no user-facing meaning: "
        "infrastructure/ETL/audit columns only (AZ_*, ETL_*, DW_*, _DMS_KEY surrogate keys used only for joining).\n"
        ...
        "Do NOT include soft-delete or record-indicator columns here — those are OPTIONAL filters "
        "that should only be applied when the user explicitly requests active or non-deleted records "
        "(e.g. 'show active customers', 'exclude deleted orders'). "
